...
} from "@/app/components/ui/card"; // adapte selon ton projet
import { Input } from "@/app/components/ui/input";
import { Label } from "@/app/components/ui/label";
import { Alert, AlertDescription } from "@/app/components/ui/alert";
import { Button } from "@/app/components/ui/button";
import { useSendVerificationCodeMutation } from "@/app/hooks/use-forgotPassword";
...
const ForgotPasswordForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
   ...
   mutate({
    ...
    onSuccess: () => {
      window.location.href = `/admin/verify-code?email=${encodeURIComponent(email)}`;
    },
    onError(error: any) {
    ...
    <Card className="border-border bg-card absolute left-1/2 top-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button onClick={() => router.push("/admin/login")} type="button" variant="ghost" className="w-full">
