...
export default function Tarifs() {
  ...
  tarifsData.forEach((tarif) => {
    allProducts.push({
      _id: tarif.id,
      nameProduct: tarif.nameAr || tarif.name,
      prix: tarif.price,
      image: tarif.image,
      ...
      <SectionMargin>
        ...
        <div className="text-center mb-8 md:mb-10">
          ...
          <p className="text-sm sm:text-base text-gray-600 max-w-2xl mx-auto">
            تصفح مجموعتنا من خدمات الغسيل والتنظيف الجاف
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5">
          {allProducts.map((product: Produit) => {
            ...
            const slugPath = `/ar/tarifs/${productSlug}`;
              ...
              >
                ...
                <div className="p-3 sm:p-4">
                  ...
                  <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-1 min-h-[2rem] line-clamp-2">
                    {product.nameProduct || "منتج بدون اسم"}
                  </h3>

                  {/* TARIFS Label */}
                  <div className="mb-1">
                    <span className="text-xs font-semibold text-orange-500">الأسعار</span>
                  </div>

                  {/* Price */}
                  <div className="mb-3">
                    <span className="text-xl sm:text-2xl font-bold text-gray-900">
                      {product.prix ? `${product.prix} درهم` : "على التقدير"}
                    </span>
                  </div>
                </div>
              </div>
