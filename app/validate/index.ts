...
export const signInSchema = z.object({
  email: z.string().email({ message: "Format de l'email invalide" }),
  password: z
    .string()
    .min(8, { message: "Mot de passe trop court ; au moins 8 caractères" }),
});

export const SignUpemployeeSchema = z
  .object({
    name: z
      .string()
...
export const resetPasswordSchema = z
  ...
  .refine((data) => data.password === data.confirmPassword, {
    message: "Les mots de passe ne correspondent pas",
    path: ["confirmPassword"], // cible l'erreur sur confirmPassword
  });
...
export const produitSchema = z.object({
  ...
  image: z
    .union([
      z.instanceof(File), // ✅ Accepts uploaded file
      z.string().min(1), // ✅ Accepts image URL or filename
    ])
    .optional(), // Make optional if needed
...
export const pickupSchema = z
  ...
  .refine(
    (data) =>
      new Date(data.dateLivraisonPrevue) >= new Date(data.dateRamassage),
    {
      message: "La date de livraison doit être après la date de ramassage",
      path: ["dateLivraisonPrevue"],
    }
  )
  .refine(
    (data) => {
      const ramassage = new Date(
        `${data.dateRamassage}T${data.heureRamassage}`
      );
      const livraison = new Date(
        `${data.dateLivraisonPrevue}T${data.heureLivraison}`
      );
      return livraison > ramassage;
    },
    {
      message:
        "La date et l'heure de livraison doivent être après le ramassage",
      path: ["dateLivraisonPrevue"],
    }
  );
